      ...
      <p class="inline-block text-lg text-gray-800 dark:text-neutral-400 font-light">
        ...
        <a href="linkedin"
          ...
          <span
            ...
            <span
              class="group-hover:underline underline-offset-2 transition-all duration-200">{{ texts?.hero?.linkedin }}</span>
          </span>
        </a>
        ...
        <a href="linkedin"
          ...
          <span
            ...
            <span
              class="group-hover:underline underline-offset-2 transition-all duration-200"> {{ texts?.hero?.downloadCV }}</span>
          </span>
        </a>
      </p>
...
export default defineComponent({
  ...
  methods: {
    t(key: string): string {
      return this.texts?.[key.split('.')[0]]?.[key.split('.')[1]] || key
    }
  },

  mounted() {
    // Log pour le debugging après montage du composant
    console.log('HerosComponent mounted - texts:', this.texts)

    // Vérifier si la clé de traduction existe
    if (!this.texts?.hero?.availableForOpportunities) {
      console.warn('⚠️ Traduction manquante: hero.availableForOpportunities')
    }
  ...
  setup() {
    // Récupérer les traductions depuis le composable
    const { texts, locale } = useI18n('home')

    // Log pour le debugging (visible dans la console du navigateur)
    console.log('HerosComponent setup - locale:', locale.value)
    console.log('HerosComponent setup - texts:', texts.value)

    // Vérifier régulièrement si les traductions sont chargées
    setTimeout(() => {
      console.log('HerosComponent timeout check - texts:', texts.value)
    }, 1000)
