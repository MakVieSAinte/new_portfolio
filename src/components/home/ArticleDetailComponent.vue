<template>
  <div
    class="box-main mx-auto pt-32 pb-10 px-8 lg:px-12 md:px-8 sm:px-5 border bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-800 !text-gray-800 !dark:text-neutral-400"
  >
    <router-link
    ...
    <h1
      v-if="article"
      class="text-4xl font-bold mt-8 mb-6 text-gray-800 dark:text-white"
    >
      {{ article.title }}
    </h1>
    ...
    try {
      ...
      if (post) {
        post.content.html = post.content.html
          ...
          .replace(
            /<h3/g,
            '<h3 class="text-xl mt-6 mb-3 font-semibold text-gray-800 dark:text-neutral-400"',
          )
          .replace(/<p/, '<p class="my-5 text-gray-800 dark:text-neutral-400"')
          .replace(
            /<p(?=\s*><code)/g,
            '<p class="mt-5 code-paragraph mb-8 text-gray-800 dark:text-neutral-400"',
          )
          .replace(
            /<pre(?=\s*><code)/g,
            '<pre class="mt-8 code-paragraph mb-8 text-gray-800 dark:text-neutral-400"',
          )
