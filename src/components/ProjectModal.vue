          ...
          <div class="p-3 md:p-5 pt-2 md:pt-3 overflow-y-auto custom-scrollbar flex-grow">
            <div>
              ...
              <div @mouseenter="showCarouselControls = true" @mouseleave="showCarouselControls = false"
                ...
                <div class="relative w-full overflow-hidden rounded-lg bg-black shadow-md" style="aspect-ratio: 16 / 9">
                  <!-- Video (if exists and is first) -->
                  <video v-if="currentMediaIsVideo && currentMediaUrl" :src="currentMediaUrl" controls playsinline
                    autoplay muted class="w-full h-full object-cover">
                    Votre navigateur ne supporte pas la balise vidéo.
                  </video>

                  <!-- Image (if current media is image) -->
                  <img v-else v-if="currentMediaUrl" :src="currentMediaUrl" :alt="project.title"
                ...
                <div v-if="allMediaItems.length > 1" class="mt-1 md:mt-3 flex gap-2 md:gap-2 overflow-x-auto pb-1">
                  <button v-for="(item, idx) in allMediaItems" :key="idx" @click="currentMediaIndex = idx" :class="[
                    ...
                    <template v-else>
                      <img :src="getMediaUrl(item.type, item.src)" v-if="getMediaUrl(item.type, item.src)"
                        :alt="project.title + ' - ' + (idx + 1)" class="w-16 h-10 md:w-20 md:h-12 object-cover" />
                    </template>
                  </button>
                </div>
              </div>
