<template>
  <transition name="fade">
    <div v-if="isOpen && project" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div
          ...
          <div>
            <div class="flex items-center gap-5 mb-0 pt-2">
              ...
              <!-- year + category -->
              <div class="text-xs text-neutral-500 dark:text-neutral-400 flex flex-wrap gap-x-1 gap-y-1">
        ...
        <div class="p-5 pt-3 overflow-y-auto custom-scrollbar flex-grow">
            ...
            <template v-else>
              ...
              <div v-if="images.length > 1" class="mt-4 flex gap-3 overflow-x-auto pb-1">
                <button v-for="(img, idx) in images" :key="idx" @click="currentIndex = idx" :class="[
                  ...
                  <img :src="img" :alt="project.title + ' - ' + (idx + 1)" class="w-24 h-16 object-cover" />
                </button>
              </div>
            </template>
          </div>
          ...
          <div class="mt-4 flex flex-col gap-4">
            <div>
              ...
              <p class="text-md text-neutral-500 dark:text-neutral-400 mt-0 italic">
                {{ project.details }}
              </p>
            </div>
        ...
        <div
          ...
          <div class="w-full flex flex-col items-center justify-center md:flex-row gap-3">
            <a v-if="project.url" :href="project.url" target="_blank" rel="noopener"
              class="flex-1 inline-flex items-center justify-center py-2 px-4 bg-primary-600 text-white text-sm font-semibold rounded-lg shadow-lg hover:bg-primary-700 transition-colors duration-200">
              Voir le projet
            </a>
...
const colorPalettes = [
  ...
  { bg: 'rgba(187, 247, 208, 0.1)', text: 'text-green-700 dark:text-green-300' },
  { bg: 'rgba(191, 219, 254, 0.1)', text: 'text-blue-700 dark:text-blue-300' },
  { bg: 'rgba(253, 230, 138, 0.1)', text: 'text-amber-700 dark:text-amber-300' },
  { bg: 'rgba(233, 213, 255, 0.1)', text: 'text-purple-700 dark:text-purple-300' },
  { bg: 'rgba(199, 210, 254, 0.1)', text: 'text-indigo-700 dark:text-indigo-300' },
