<template>
  ...
  >
    ...
    <div
      v-if="article"
      class="sticky bottom-6 inset-x-0 text-center max-md:hidden"
    >
      <div
  ...
  methods: {
    optimizeHTML(html: string): string {
      ...
      optimized = optimized
        ...
        .replace(
          /<p(?=\s*><code)/g,
          '<p class="mt-5 code-paragraph mb-8 w-full"',
        )
        .replace(
          /<pre(?=\s*><code)/g,
          '<pre class="mt-8 code-paragraph mb-8 w-full overflow-x-auto"',
        )
        .replace(
          /<blockquote/g,
          '<blockquote class="my-4 pl-4 border-l-4 w-full"',
        )
